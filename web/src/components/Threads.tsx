...
const Threads: React.FC<ThreadsProps> = ({
  ...
  // Synced after render rather than during it (refs must not be written while
  // rendering). The animation loop reads this every frame, so it picks up new
  // props on the next one.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Respect reduced motion: draw a single still frame and stop, so the lines
    // stay as texture without moving. Re-checked live if the setting changes.
    const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)');
    // A fixed moment in the noise field that reads well as a still.
    const STILL_TIME_MS = 18_000;

    function update(t: number) {
      if (reducedMotion.matches) {
        program.uniforms.iTime.value = STILL_TIME_MS * 0.001;
    ...
    function handleMotionChange() {
      if (!reducedMotion.matches) animationFrameId.current = requestAnimationFrame(update);
    }
  ...
  return <div ref={containerRef} aria-hidden="true" className="w-full h-full relative" {...rest} />;
};
